// Main applications logic for Bridge Scoring Calculator
document.addEventListener('DOMContentLoaded', function() {
    ...
    const breakdownContent = document.getElementById('breakdown// Main applications logic for Bridge Scoring Calculator
document.addEventListener('DOMContentLoaded', function() {
    // DOM Elements
    const contractDisplay = document.getElementById('contractDisplay');
    const contractStateDisplay = document.getElementById('contractStateDisplay');
    ...
    function setTricks(trickChange) {
        ...
        if (trickButton.classList.contains('selected')) {
            ...
            currentTricks = currentLevel + 6; // This resets to making contract if already selected
        } else {
    ...
    function calculate() {
        try {
            ...
            if (currentTricks === null) {
                currentTricks = currentLevel + 6; // Default to making contract if tricks not set
            }
            ...
            console.log('currentTricks (made):', currentTricks);
            console.log('isVulnerable:', isVulnerable);
            console.log('doubledState:', doubledState);
            console.log('doubled (param):', doubledState === 'doubled');
            console.log('redoubled (param):', doubledState === 'redoubled');
            console.log('Expected contract tricks:', currentLevel + 6);
            console.log('Undertricks (if any):', (currentLevel + 6) - currentTricks);
            // --- DEBUGGING LOGS END ---
    ...
    function showBreakdown() {
        if (!currentLevel || !currentSuit) {
            ...
            const message = "Please select a complete contract first.";
        ...
        calculate(); // Ensure score is calculated before showing breakdown
        ...
        if (currentTricks >= contractTricks) {
            ...
            // Undetricks scenario
            const undertricks = contractTricks - currentTricks;
            content += `<div>Contract: ${currentLevel}${currentSuit} ${doubledState !== 'none' ? (doubledState === 'doubled' ? 'X' : 'XX') : ''}</div>`;
            content += `<div>Vulnerable: ${isVulnerable ? 'Yes' : 'No'}</div>`;
            content += `<div>Tricks Made: ${currentTricks} (out of ${contractTricks})</div>`;
            content += `<div>Down By: ${undertricks}</div>`;
            content += `<div>Penalty: ${resultDisplay ? resultDisplay.textContent : 'N/A'}</div>`;
            // For a more detailed breakdown of penalties, you might need to expose more from BridgeScoring.js
            // or re-calculate parts here based on the rules.
        }
    ...
    function switchToCurrencyCalculator() {
        ...
        if (currencyCalc) {
            ...
            displayMessageBox("Currency calculator coming soon!");
        }
    }
...
// Updated with Keep Awake feature and debugging logs for scoring
Content');
    const closeModal = document.querySelector('.close');
...
// Updated with Keep Awake feature
